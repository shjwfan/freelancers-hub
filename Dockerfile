FROM openjdk:17.0-jdk

ENV HOME /freelancers-hub

WORKDIR "$HOME"

ARG JAR=web/target/web-1.0-SNAPSHOT.jar
ARG JAR_PROPS=web/src/main/resources/application.yaml
ARG LOGGING_CONFIG=web/src/main/resources/logging-config.xml

COPY ${JAR} web-1.0-SNAPSHOT.jar
COPY ${JAR_PROPS} application.yaml
COPY ${LOGGING_CONFIG} logging-config.xml

ENTRYPOINT ["java", "-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005", "-jar", "/freelancers-hub/web-1.0-SNAPSHOT.jar", "./", "--spring.config.import=file:/freelancers-hub/application.yaml"]
